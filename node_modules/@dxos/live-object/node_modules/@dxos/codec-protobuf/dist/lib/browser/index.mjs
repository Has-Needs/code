...
var sanitize = (type, value, path, context) => {
  ...
  for (const key of Object.keys(value)) {
    ...
    if (field.resolvedType instanceof pb2.Type) {
      sanitize(field.resolvedType, value[key], `${path}.${key}`, context);
    } else if (field.resolvedType instanceof pb2.Enum) {
...
var Service = class {
  constructor(backend, service, schema, encodingOptions) {
    for (const method of service.methodsArray) {
      method.resolve();
      invariant3(method.resolvedRequestType, void 0, {
        F: __dxlog_file3,
        L: 59,
        S: this,
        A: [
      ...
      invariant3(method.resolvedResponseType, void 0, {
        F: __dxlog_file3,
        L: 60,
        S: this,
        A: [
      ...
      invariant3(!method.requestStream, "Streaming RPC requests are not supported.", {
        F: __dxlog_file3,
        L: 61,
        S: this,
        A: [
...
var ServiceHandler = class {
  ...
  */
  async call(methodName, request, options) {
    ...
    invariant3(!method.requestStream, "Invalid RPC method call: request streaming mismatch.", {
      F: __dxlog_file3,
      L: 120,
      S: this,
      A: [
    ...
    invariant3(!method.responseStream, `Invalid RPC method call: response streaming mismatch. ${methodName}`, {
      F: __dxlog_file3,
      L: 121,
      S: this,
      A: [
  ...
  */
  ...
  async _getHandler(method) {
    const service = await getAsyncProviderValue(this._serviceProvider);
    const handler = service[method];
    invariant3(handler, `Handler is missing: ${method}`, {
      F: __dxlog_file3,
      L: 163,
      S: this,
      A: [
  ...
  _getMethodInfo(methodName) {
    ...
    invariant3(!!method, `Method not found: ${methodName}`, {
      F: __dxlog_file3,
      L: 169,
      S: this,
      A: [
...
var Schema = class _Schema {
  static fromJson(schema, substitutions = {}) {
    const root = pb3.Root.fromJSON(schema);
  ...
  */
  addJson(schema) {
    ...
    this._typesRoot = pb3.Root.fromJSON(merge(this._typesRoot.toJSON(), schema));
  }
};
