...
export type PhysicalSeries<K extends string = string, S extends Series<any> = Series> = Partial<Record<K, S>>;
...
export type PhysicalLayer<K extends string = string, S extends Series<any> = Series, P extends string = string> = {
    conditions: Conditions<K>;
    series: Record<P, Partial<PhysicalSeries<K, S>>>;
    definitions?: Definitions<S>;
    namespace?: string;
};
export type SemanticLayer<K extends string = string, P extends string = string, V = number, Q extends string = string> = {
    conditions: Conditions<K>;
...
export type SememeAnnotation = {
    sememeName: string;
    conditionId: string;
};
export type SemanticValues<P extends string = string, V = number> = Record<P, SemanticAnnotatedValues<V>>;
export type SemanticAnnotatedValues<V = number> = Map<V, SememeAnnotation[]>;
export type FacetAnnotatedValues<V = number> = Map<V, {
    physical: ('values' | 'naming')[];
    semantic: SememeAnnotation[];
}>;
type RequiresExtendsIfNotProvided<T> = T | (Partial<T> & {
    extends: string;
});
/**
 ...
 */
export type Conditions<K extends string = string> = Partial<Record<K, Statements>>;
export type AccompanyingSeries = Pick<LinearSeries, 'slope' | 'initial' | 'extends'> & RequiresExtendsIfNotProvided<AccompanyingSeriesConfig>;
export type ResolvedAccompanyingSeries = Pick<ResolvedLinearSeries, 'slope' | 'initial'> & AccompanyingSeriesConfig;
 ...
 */
export type LinearSeries = Series & RequiresExtendsIfNotProvided<LinearSeriesConfig>;
 ...
 */
export type ExponentialSeries = Series & RequiresExtendsIfNotProvided<ExponentialSeriesConfig>;
 ...
 */
export type HelicalArcSeries = Series<HelicalArcValue> & RequiresExtendsIfNotProvided<HelicalArcSeriesConfig>;
export type ResolvedHelicalArcSeries = HelicalArcSeries & HelicalArcSeriesConfig;
 ...
 */
export type ValueOfSeries<S extends Series<any> = Series> = NonNullable<S['values']>[number];
/**
 ...
 */
export type AuditTokens<S extends Series<any> = Series> = (auditProps: AuditTokensParams<S>, ...definitions: Definitions<Series<any>>[]) => TokenAudit<S>[];
/**
 ...
 */
export type RenderTokens<S extends Series<any> = Series> = (renderProps: RenderTokensParams<S>, ...definitions: Definitions<Series<any>>[]) => string[];
