...
__export(src_exports, {
  arcToConstellation: () => arcToConstellation,
  constellationFromHelicalArc: () => constellationFromHelicalArc,
  cssGradientFromCurve: () => cssGradientFromCurve,
  getLinearSpace: () => getLinearSpace,
  getOklabVectorsFromLuminosities: () => getOklabVectorsFromLuminosities,
  helicalArcFromConfig: () => helicalArcFromConfig,
  oklabVectorToValue: () => oklabVectorToValue,
  parseAlphaLuminosity: () => parseAlphaLuminosity
});
module.exports = __toCommonJS(src_exports);

// packages/colors/src/util/geometry.ts
...
function getPointsOnCurve(curve, divisions) {
  ...
  for (let d2 = 0; d2 <= divisions; d2++) {
    points.push(getPointOnCurve(curve, d2 / divisions));
  }
  return points;
}
function arcToConstellation(curvePath, divisions = curveResolution) {
  const points = [];
  let last2;
...
// packages/colors/src/util/palettes.ts
var getLinearSpace = (min, max2, n2) => {
  const result = [];
  const delta = (max2 - min) / n2;
  for (let i = 0; i < n2; i++) {
...
var alphaPattern = /\//;
var parseAlphaLuminosity = (alphaLuminosity) => {
  if (typeof alphaLuminosity === "number") {
    return [alphaLuminosity, void 0];
  } else {
    const [luminosity, alpha] = alphaLuminosity.split(alphaPattern);
    return [parseFloat(luminosity), parseFloat(alpha)];
  }
};
var getOklabVectorsFromLuminosities = (luminosities, constellation) => {
  let c4 = 0;
  const result = [];
  for (let i = 0; i < luminosities.length; i++) {
    const [l] = parseAlphaLuminosity(luminosities[i]);
    while (constellation[c4 + 1] && l > constellation[c4 + 1][0]) {
      c4++;
    }
    const [l1, a1, b1] = constellation[c4];
    const [l2, a2, b2] = constellation[c4 + 1];
    const u = (l - l1) / (l2 - l1);
    result.push([
      l1 + (l2 - l1) * u,
      a1 + (a2 - a1) * u,
...
function constellationFromHelicalArc(curve, curveDepth = 32) {
  return arcToConstellation(
    curve,
    Math.ceil(curveDepth * (1 + Math.abs(curve.torsion || 1)) / 2)
  ).map(
    (curvePoint) => applyTorsion(curvePoint, curve.torsion, curve.torsionT0)
  );
}
function oklabVectorToValue(Lab, gamut, alpha) {
  return new Color("oklab", Lab, alpha).to(gamut).toString({ inGamut: true });
}
function constellationToValues(constellation, gamut) {
  return constellation.map((shade) => oklabVectorToValue(shade, gamut));
}
function applyTorsion(pointOnCurve, torsion = 0, torsionT0 = 0.5) {
  const t = pointOnCurve[0];
  const [l, c4, h] = new Color("oklab", pointOnCurve).to("oklch").oklch;
  const hueOffset = torsion * (t - torsionT0);
  return Array.from(
    new Color("oklch", [l, c4, h + hueOffset]).to("oklab").oklab
  );
}
function helicalArcFromConfig({
  keyPoint,
  lowerCp,
  upperCp,
  torsion
  ...
  const [l, a2, b2] = keyColor.to("oklab").oklab;
  const keyColorPosition = [l, a2, b2];
  const darkControlPosition = [l * (1 - lowerCp), a2, b2];
  const lightControlPosition = [l + (1 - l) * upperCp, a2, b2];
  return {
    curves: [
...
function cssGradientFromCurve(curve, nShades = 21, range2 = [0, 1], gamut = "srgb", curveDepth = 32) {
  return `linear-gradient(to right, ${constellationToValues(
    getOklabVectorsFromLuminosities(
      getLinearSpace(range2[0], range2[1], nShades),
