...
export const App: React.FC = () => {
  ...
  function handlePersonaSelect(persona: Persona) {
    ...
    // Don't open modal anymore - just set active persona
  }
  ...
  // Active persona state (for showing Communities button and Has/Needs)
  ...
  return (
    <div style={{ 
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              {isAgregore ? (
                p2pStatus === 'p2p' ? '⚡' : p2pStatus === 'offline' ? 'OFF' : '🌐'
              ) : (
                p2pStatus === 'offline' ? 'OFF' : '🌐'
              )}
            </div>
          ...
          {getVisiblePersonas().slice(personaScrollIndex, personaScrollIndex + 5).map((persona, index) => (
            <button
              ...
              aria-label={`View ${persona.name} details`}
              onMouseEnter={(e) => {
          ...
          {/* Communities Button - shows when persona is selected */}
          {activePersona && (
            <div style={{
              display: 'flex',
              alignItems: 'center',
              ...
              <button
                ...
                title={`Manage communities for ${activePersona.name}`}
                aria-label={`Manage communities for ${activePersona.name}`}
                onMouseEnter={(e) => {
              ...
              >
                🏘️ Communities ({activePersona.communities.length})
              </button>
            </div>
          )}
      ...
      {/* Communities Modal */}
        ...
        Communities for {activePersona.name}
        </h3>
        <button
        onClick={() => {
        ...
        onMouseEnter={(e) => {
        e.currentTarget.style.background = 'linear-gradient(135deg, #e0a800, #d39e00)';
        e.currentTarget.style.transform = 'translateY(-1px)';
        }}
        onMouseLeave={(e) => {
        e.currentTarget.style.background = 'linear-gradient(135deg, #ffc107, #e0a800)';
        e.currentTarget.style.transform = 'translateY(0)';
        }}
        >
        🔗 Create Subcommunity
        ...
        {activePersona.communities.length === 0 ? (
        <div style={{
        gridColumn: '1 / -1',
        textAlign: 'center',
        color: '#666',
        ...
        <p>Create your first community using the buttons above!</p>
        </div>
        ) : (
        activePersona.communities.map((community) => (
        <div
        key={community.id}
        style={{
        padding: '16px',
        ...
        <div>Messages: {community.messages.length}</div>
        </div>
        </div>
        ))
        )}
