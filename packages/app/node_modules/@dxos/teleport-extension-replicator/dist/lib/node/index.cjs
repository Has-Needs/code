...
__export(node_exports, {
  ReplicatorExtension: () => ReplicatorExtension
});
...
var ReplicatorExtension = class {
  constructor() {
    ...
    this._streams = new import_util.ComplexMap(import_keys.PublicKey.hash);
    this._options = {
      upload: false
    };
    this._updateTask = new import_async.DeferredTask(this._ctx, async () => {
      try {
  ...
  async onOpen(context) {
    ...
    this._rpc = (0, import_rpc.createProtoRpcPeer)({
      requested: {
        ReplicatorService: import_proto.schema.getService("dxos.mesh.teleport.replicator.ReplicatorService")
      },
      exposed: {
        ReplicatorService: import_proto.schema.getService("dxos.mesh.teleport.replicator.ReplicatorService")
      },
      handlers: {
        ReplicatorService: {
          updateFeeds: async ({ feeds }) => {
            (0, import_log.log)("received feed info", {
              feeds
            }, {
              F: __dxlog_file,
              L: 108,
            ...
            (0, import_invariant.invariant)(this._extensionContext.initiator === true, "Invalid call", {
              F: __dxlog_file,
              L: 109,
              S: this,
              A: [
          ...
          startReplication: async ({ info }) => {
            (0, import_log.log)("starting replication...", {
              info
            }, {
              F: __dxlog_file,
              L: 113,
            ...
            (0, import_invariant.invariant)(this._extensionContext.initiator === false, "Invalid call", {
              F: __dxlog_file,
              L: 114,
              S: this,
              A: [
          ...
          stopReplication: async ({ info }) => {
            (0, import_log.log)("stopping replication...", {
              info
            }, {
              F: __dxlog_file,
              L: 122,
  ...
  async _reevaluateFeeds() {
    (0, import_log.log)("_reevaluateFeeds", void 0, {
      F: __dxlog_file,
      L: 161,
      S: this,
      C: (f, a) => f(...a)
    ...
    for (const feedKey of this._feeds.keys()) {
      ...
      if (this._streams.has(feedKey) && this._options.upload !== this._streams.get(feedKey)?.info.upload) {
        try {
          await (0, import_async.asyncTimeout)(this._stopReplication(feedKey), 1e3);
        } catch (err) {
  ...
  */
  async _initiateReplication(feedInfo) {
    (0, import_log.log)("initiating replication", {
      feedInfo
    }, {
      F: __dxlog_file,
      L: 191,
    ...
    (0, import_invariant.invariant)(!this._streams.has(feedInfo.feedKey), `Replication already in progress for feed: ${feedInfo.feedKey}`, {
      F: __dxlog_file,
      L: 193,
      S: this,
      A: [
  ...
  */
  async _acceptReplication(feedInfo) {
    ...
    const tag = `feed-${feedInfo.feedKey.toHex()}-${import_keys.PublicKey.random().toHex().slice(0, 8)}`;
    await this._replicateFeed(feedInfo, tag);
    return tag;
  }
  async _replicateFeed(info, streamTag) {
    (0, import_log.log)("replicate", {
      info,
      streamTag
    }, {
      F: __dxlog_file,
    ...
    (0, import_invariant.invariant)(!this._streams.has(info.feedKey), `Replication already in progress for feed: ${info.feedKey}`, {
      F: __dxlog_file,
      L: 221,
      S: this,
      A: [
    ...
    const feed = this._feeds.get(info.feedKey) ?? (0, import_debug.failUndefined)();
    ...
    replicationStream.on("error", (err) => {
      if (err instanceof import_protocols.TimeoutError) {
        import_log.log.info("replication stream timeout", {
          err,
          info
        }, {
          F: __dxlog_file,
      ...
      if (err?.message === "Writable stream closed prematurely" || err?.message === "Cannot call write after a stream was destroyed") {
        (0, import_log.log)("replication stream closed", {
          err,
          info
        }, {
          F: __dxlog_file,
      ...
      if (replicationStreamErrors === 0) {
        import_log.log.info("replication stream error", {
          err,
          info
        }, {
          F: __dxlog_file,
